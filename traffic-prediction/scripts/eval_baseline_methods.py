...
def main(args):
    ext = os.path.splitext(args.traffic_reading_filename)[-1].lower()
    print(ext)
    if ext == '.h5':
        traffic_reading_df = pd.read_hdf(args.traffic_reading_filename)
    elif ext == '.npy':
        traffic_reading_df = np.load(args.traffic_reading_filename)
        traffic_reading_df = pd.DataFrame(traffic_reading_df)
    # traffic_reading_df = pd.read_hdf(args.traffic_reading_filename)
